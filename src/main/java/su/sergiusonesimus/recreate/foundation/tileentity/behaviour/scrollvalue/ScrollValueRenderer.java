...
public class ScrollValueRenderer {
	...
	public static void tick() {
		...
		ScrollValueBehaviour behaviour = TileEntityBehaviour.get(world, target.blockX, target.blockY, target.blockZ, ScrollValueBehaviour.TYPE);
		if (behaviour == null)
			return;
...
//			return;
		...
		if (behaviour instanceof BulkScrollValueBehaviour && AllKeys.ctrlDown()) {
			...
			for (SmartTileEntity smartTileEntity : bulkScrolling.getBulk()) {
				...
				if (other != null)
					addBox(world, smartTileEntity.xCoord, smartTileEntity.yCoord, smartTileEntity.zCoord, face, other,
							highlight);
			}
		} else
			addBox(world, target.blockX, target.blockY, target.blockZ, face, behaviour, highlight);
	}

	protected static void addBox(WorldClient world, int x, int y, int z, Direction face, ScrollValueBehaviour behaviour,
		boolean highlight) {
		float inflator = .5f;
		AxisAlignedBB bb = AxisAlignedBB.getBoundingBox(0, 0, 0, 0, 0, 0).expand(inflator, inflator, inflator / 4)
		...
		if (behaviour instanceof ScrollOptionBehaviour) {
			...
			if (behaviour.unit != null)
				box.subLabel(new ChatComponentText("(").appendSibling(behaviour.unit.apply(behaviour.scrollableValue))
						.appendText(")"));
		}

		box.scrollTooltip(new ChatComponentText("[").appendSibling(Lang.translate("action.scroll")).appendText("]"));
