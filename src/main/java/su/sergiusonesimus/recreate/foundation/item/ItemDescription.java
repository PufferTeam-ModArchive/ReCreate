...
public class ItemDescription {
    ...
    public static IChatComponent trim = new ChatComponentText("                          ").setChatStyle(
        new ChatStyle().setColor(WHITE)
    ...
    public static List<IChatComponent> getKineticStats(Block block) {
        ...
        boolean hasStressImpact = StressImpact.isEnabled() && showStressImpact
        ...
        if (hasSpeedRequirement) {
            List<IChatComponent> speedLevels = Lang
                .translatedOptions("tooltip.speedRequirement", "none", "medium", "high");
        ...
        if (hasStressCapacity) {
            List<IChatComponent> stressCapacityLevels = Lang
                .translatedOptions("tooltip.capacityProvided", "low", "medium", "high");
    ...
    public ItemDescription withBehaviour(String condition, String behaviour) {
        ...
        conditionComponent.getChatStyle()
            .setColor(GRAY);
    ...
    public ItemDescription createTabs() {
        ...
        if (hasDescription || hasControls) {
            ...
            for (List<IChatComponent> list : Arrays.asList(lines, linesOnShift, linesOnCtrl)) {
                ...
                if (shift || ctrl) list.add(hasDescription && hasControls ? 2 : 1, new ChatComponentText(""));
            }
        }

        if (!hasDescription) linesOnShift = lines;
        if (!hasControls) linesOnCtrl = lines;
