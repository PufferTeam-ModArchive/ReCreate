...
public class TooltipHelper {
	...
	public static IChatComponent holdShift(Palette color, boolean highlighted) {
		...
		keyShift.getChatStyle().setColor(EnumChatFormatting.GRAY);
		IChatComponent holdForDescription = Lang.translate("tooltip.holdForDescription", keyShift);
		holdForDescription.getChatStyle().setColor(EnumChatFormatting.DARK_GRAY);
	...
	public static void addHint(List<IChatComponent> tooltip, String hintKey, Object... messageParams) {
		...
		spacingCopy.getChatStyle().setColor(EnumChatFormatting.GOLD);
		tooltip.add(spacingCopy);
		IChatComponent hint = Lang.translate(hintKey);
		List<IChatComponent> cutComponent = TooltipHelper.cutTextComponent(hint, EnumChatFormatting.GRAY, EnumChatFormatting.WHITE);
		for (IChatComponent component : cutComponent) {
			tooltip.add(TextHelper.plainCopy(spacing).appendSibling(component));
		}
	}

	public static void referTo(Item item, int meta, Supplier<? extends Item> itemWithTooltip) {
		referTo(new ItemStack(item, 1, meta), itemWithTooltip.get().getUnlocalizedName());
	}

	public static void referTo(Block block, int meta, Supplier<? extends Item> itemWithTooltip) {
		referTo(new ItemStack(block, 1, meta), itemWithTooltip.get().getUnlocalizedName());
	}
	...
	@Deprecated
	public static List<String> cutString(IChatComponent s, EnumChatFormatting defaultColor, EnumChatFormatting highlightColor) {
	...
	public static List<IChatComponent> cutTextComponent(IChatComponent c, EnumChatFormatting defaultColor,
		...
		lineStart.getChatStyle().setColor(defaultColor);
		...
		for (String string : lines) {
			...
			for (String part : split) {
				...
				partComponent.getChatStyle().setColor(f.get(currentlyHighlighted));
...
//		}
	...
	private static ItemDescription buildToolTip(String translationKey, ItemStack stack) {
		...
		for (int i = 1; i < 100; i++) {
			...
			if (!StatCollector.canTranslate(conditionKey))
				break;
			if (i == 1)
				tooltip.getLinesOnShift()
					.add(new ChatComponentText(""));
			tooltip.withBehaviour(StatCollector.translateToLocal(conditionKey), StatCollector.translateToLocal(behaviourKey));
		}
		...
		for (int i = 1; i < 100; i++) {
			...
			if (!StatCollector.canTranslate(controlKey))
				break;
