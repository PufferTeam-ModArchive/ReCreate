...
public class PlacementOffset {
	...
	public static PlacementOffset success(ChunkCoordinates pos, Function<Pair<Block, Integer>, Pair<Block, Integer>> transform) {
	...
	public boolean isReplaceable(World world) {
		...
		return world.getBlock(pos.posX, pos.posY, pos.posZ).getMaterial().isReplaceable();
	}
	...
	public boolean placeInWorld(World world, ItemBlock itemBlock, EntityPlayer player, MovingObjectPosition ray) {
		...
		if (!player.canPlayerEdit(newX, newY, newZ, ray.sideHit, player.getHeldItem()))
			return false;

		Pair<Block, Integer> state = stateTransform.apply(Pair.of(itemBlock.field_150939_a, player.getHeldItem().getItemDamage()));
		...
		world.playSoundEffect(newX, newY, newZ, soundtype.getBreakSound(), (soundtype.getVolume() + 1.0F) / 2.0F, soundtype.getPitch() * 0.8F);
