...
public class ChasingAABBOutline extends AABBOutline {
    ...
    private static AxisAlignedBB interpolateBBs(AxisAlignedBB current, AxisAlignedBB target, float pt) {
        return AxisAlignedBB.getBoundingBox(
            ReCreateMath.lerp(pt, current.minX, target.minX),
            ReCreateMath.lerp(pt, current.minY, target.minY),
            ReCreateMath.lerp(pt, current.minZ, target.minZ),
            ReCreateMath.lerp(pt, current.maxX, target.maxX),
            ReCreateMath.lerp(pt, current.maxY, target.maxY),
