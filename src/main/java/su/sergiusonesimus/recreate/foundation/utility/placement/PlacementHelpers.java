...
public class PlacementHelpers {
    ...
    public static void tick() {
        ...
        if (targetX == null || targetY == null || targetZ == null) {
            if (animationTick > 0) animationTick = Math.max(animationTick - 2, 0);

            return;
        }

        if (animationTick < 10) animationTick++;
    ...
    private static void checkHelpers() {
        ...
        if (hitResult == null || hitResult.typeOfHit != MovingObjectType.BLOCK) return;

        if (mc.thePlayer == null) return;

        if (mc.thePlayer.isSneaking())// for now, disable all helpers when sneaking TODO add helpers that respect
        ...
        if (filteredForHeldItem.isEmpty()) return;
        ...
        List<IPlacementHelper> filteredForState = filteredForHeldItem.stream()
            .filter(helper -> helper.matchesBlock(block, meta))
            .collect(Collectors.toList());
        if (filteredForState.isEmpty()) return;

        for (IPlacementHelper h : filteredForState) {
            PlacementOffset offset = h
                .getOffset(mc.thePlayer, world, block, meta, posX, posY, posZ, hitResult, heldItem);
    ...
    private static void drawDirectionIndicator(float centerX, float centerY, float progress, float partialTicks) {
        ...
        Vec3 projTarget = VecHelper
            .projectToPlayerView(VecHelper.getCenterOf(lastTargetX, lastTargetY, lastTargetZ), partialTicks);
        ...
        if (animationTick < 10) angle.set(targetAngle);
        ...
        else if (mode == CClient.PlacementIndicatorSetting.TEXTURE) textured(centerX, centerY, a, snappedAngle);
    }

    private static void fadedArrow(float centerX, float centerY, float r, float g, float b, float a, float length,
        float snappedAngle) {
    ...
    private static void textured(float centerX, float centerY, float alpha, float snappedAngle) {
        ...
        Minecraft.getMinecraft()
            .getTextureManager()
            .bindTexture(AllGuiTextures.PLACEMENT_INDICATOR_SHEET.location);
