...
public class AABBOutline extends Outline {
	...
	protected void renderFace(ForgeDirection direction, Vec3 p1, Vec3 p2, Vec3 p3, Vec3 p4, boolean noCull) {
	    ...
	    params.alpha = (direction == params.getHighlightedFace().toForgeDirection() && params.hightlightedFaceTexture.isPresent()) 
	        ? 1.0f : 0.5f;

	    params.faceTexture.get().bind();
	    ...
	    Vec3 uDiff = Vec3.createVectorHelper(
	        p2.xCoord - p1.xCoord,
	        p2.yCoord - p1.yCoord,
	        p2.zCoord - p1.zCoord
	    );
	    Vec3 vDiff = Vec3.createVectorHelper(
	        p4.xCoord - p1.xCoord,
	        p4.yCoord - p1.yCoord,
	        p4.zCoord - p1.zCoord
	    );
