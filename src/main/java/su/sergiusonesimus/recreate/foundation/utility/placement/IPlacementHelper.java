...
public interface IPlacementHelper {
	 ...
	 */
	PlacementOffset getOffset(EntityPlayer player, World world, Block block, int meta, int x, int y, int z, MovingObjectPosition ray);
	...
	default PlacementOffset getOffset(EntityPlayer player, World world, Block block, int meta, int x, int y, int z, MovingObjectPosition ray, ItemStack heldItem) {
	 ...
	 * @param meta the metadata of the Block that the player is looking at or clicked on
	 * @param ray the exact raytrace result
	 * @param offset the PlacementOffset returned by {@link #getOffset(EntityPlayer, World, Block, int, int, int, int, MovingObjectPosition)}<br>
	 *               the offset will always be successful if this method is called
	 */
	default void renderAt(int x, int y, int z, Block block, int meta, MovingObjectPosition ray, PlacementOffset offset) {
		displayGhost(offset);
	}
	...
	static void renderArrow(Vec3 center, Vec3 target, Direction arrowPlane, double distanceFromCenter) {
		...
		Vec3 offset = Vec3.createVectorHelper(direction.xCoord * multiplier, direction.yCoord * multiplier, direction.zCoord * multiplier);
		multiplier = .25;
		Vec3 offsetA = direction.crossProduct(facing).normalize();
		offsetA = Vec3.createVectorHelper(offsetA.xCoord * multiplier, offsetA.yCoord * multiplier, offsetA.zCoord * multiplier);
		Vec3 offsetB = facing.crossProduct(direction).normalize();
		offsetB = Vec3.createVectorHelper(offsetA.xCoord * multiplier, offsetA.yCoord * multiplier, offsetA.zCoord * multiplier);
		multiplier = .75;
		Vec3 scaledDirection = Vec3.createVectorHelper(direction.xCoord * multiplier, direction.yCoord * multiplier, direction.zCoord * multiplier);
		Vec3 endA = center.addVector(scaledDirection.xCoord + offsetA.xCoord, scaledDirection.yCoord + offsetA.yCoord, scaledDirection.zCoord + offsetA.zCoord);
		Vec3 endB = center.addVector(scaledDirection.xCoord + offsetB.xCoord, scaledDirection.yCoord + offsetB.yCoord, scaledDirection.zCoord + offsetB.zCoord);
		ClientProxy.OUTLINER.showLine("placementArrowA" + center + target, start.addVector(offset.xCoord, offset.yCoord, offset.zCoord), endA.addVector(offset.xCoord, offset.yCoord, offset.zCoord)).lineWidth(1 / 16f);
		ClientProxy.OUTLINER.showLine("placementArrowB" + center + target, start.addVector(offset.xCoord, offset.yCoord, offset.zCoord), endB.addVector(offset.xCoord, offset.yCoord, offset.zCoord)).lineWidth(1 / 16f);
	}

	default void displayGhost(PlacementOffset offset) {
		if (!offset.hasGhostBlock())
			return;

		Pair<Block, Integer> transformedPair = offset.getTransform().apply(Pair.of(offset.getGhostBlock(), offset.getGhostMeta()));
	...
	static List<Direction> orderedByDistanceOnlyAxis(int x, int y, int z, Vec3 hit, Direction.Axis axis, Predicate<Direction> includeDirection) {
		return orderedByDistance(x, y, z, hit, ((Predicate<Direction>) dir -> dir.getAxis() == axis).and(includeDirection));
	}
	...
	static List<Direction> orderedByDistanceExceptAxis(int x, int y, int z, Vec3 hit, Direction.Axis axis, Predicate<Direction> includeDirection) {
		return orderedByDistance(x, y, z, hit, ((Predicate<Direction>) dir -> dir.getAxis() != axis).and(includeDirection));
	}

	static List<Direction> orderedByDistanceExceptAxis(int x, int y, int z, Vec3 hit, Direction.Axis first, Direction.Axis second) {
		return orderedByDistanceExceptAxis(x, y, z, hit, first, d -> d.getAxis() != second);
	}

	static List<Direction> orderedByDistanceExceptAxis(int x, int y, int z, Vec3 hit, Direction.Axis first, Direction.Axis second, Predicate<Direction> includeDirection) {
		return orderedByDistanceExceptAxis(x, y, z, hit, first, ((Predicate<Direction>) d -> d.getAxis() != second).and(includeDirection));
	}
	...
	static List<Direction> orderedByDistance(int x, int y, int z, Vec3 hit, Predicate<Direction> includeDirection) {
		Vec3 centerToHit = VecHelper.getCenterOf(x, y, z).subtract(hit);
		return Arrays.stream(Iterate.directions)
				.filter(includeDirection)
				.map(dir -> {
					ChunkCoordinates dirNormal = dir.getNormal();
					return Pair.of(dir, Vec3.createVectorHelper(dirNormal.posX, dirNormal.posY, dirNormal.posZ).distanceTo(centerToHit));
				})
