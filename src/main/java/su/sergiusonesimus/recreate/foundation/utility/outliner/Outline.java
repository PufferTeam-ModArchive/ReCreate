...
public abstract class Outline {
	...
	public void renderCuboidLine(Vec3 start, Vec3 end) {
	    Vec3 diff = Vec3.createVectorHelper(end.xCoord - start.xCoord, end.yCoord - start.yCoord, end.zCoord - start.zCoord);
	    
	    float hAngle = (float) Math.toDegrees(Math.atan2(diff.xCoord, diff.zCoord));
	    float hDistance = (float) Vec3.createVectorHelper(diff.xCoord, 0, diff.zCoord).lengthVector();
	...
	public void renderAACuboidLine(Vec3 start, Vec3 end) {
		...
		extension = Vec3.createVectorHelper(extension.xCoord * modifier, extension.yCoord * modifier,
				extension.zCoord * modifier);
	...
	public void putQuadUV(Vec3 v1, Vec3 v2, Vec3 v3, Vec3 v4, float minU,
		float minV, float maxU, float maxV, ForgeDirection normal) {
