...
public abstract class PoleHelper<T extends Comparable<T>> implements IPlacementHelper {
	...
	public PoleHelper(Predicate<Pair<Block, Integer>> blockPredicate, Function<Pair<Block, Integer>, Axis> axisFunction) {
		this.blockPredicate = blockPredicate;
		this.axisFunction = axisFunction;
	}

	public boolean matchesAxis(Block block, int meta, Axis axis) {
		if (!blockPredicate.test(Pair.of(block, meta)))
			return false;
	...
	public PlacementOffset getOffset(EntityPlayer player, World world, Block block, int meta, int x, int y, int z,
		...
		List<Direction> directions = IPlacementHelper.orderedByDistance(x, y, z, ray.hitVec,
				dir -> dir.getAxis() == axisFunction.apply(Pair.of(block, meta)));
