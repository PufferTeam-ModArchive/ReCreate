...
public enum AllGuiTextures implements ScreenElement {
	...
	private AllGuiTextures(String location, int startX, int startY, int width, int height) {
		this(ReCreate.ID, location, startX, startY, width, height);
	}
	...
	public void render(int x, int y) {
		bind();
	    Gui.func_146110_a(x, y, startX, startY, width, height, 256, 256);
	}

	@SideOnly(Side.CLIENT)
	public void render(int x, int y, Gui gui) {
		bind();
	    gui.drawTexturedModalRect(x, y, startX, startY, width, height);
	}

	@SideOnly(Side.CLIENT)
	public void render(int x, int y, Color c) {
		bind();
		UIRenderHelper.drawColoredTexture(c, x, y, startX, startY, width, height);
	}
