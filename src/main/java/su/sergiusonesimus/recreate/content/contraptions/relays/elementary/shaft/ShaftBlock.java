...
public class ShaftBlock extends AbstractShaftBlock implements ITE<ShaftTileEntity> {
	...
	@Override
    public boolean onBlockActivated(World worldIn, int x, int y, int z, EntityPlayer player, int side, float subX, float subY,
    		float subZ) {
...
//		}
		...
		if (helper.matchesItem(heldItem)) {
			...
			return helper.getOffset(player, worldIn, this, meta, x, y, z, ray).placeInWorld(worldIn,
					(ItemBlock) heldItem.getItem(), player, ray);
		}
	...
	private static class PlacementHelper extends PoleHelper<Axis> {
		...
		public Predicate<ItemStack> getItemPredicate() {
			return i -> i != null && i.getItem() instanceof ItemBlock && ((ItemBlock) i.getItem()).field_150939_a instanceof AbstractShaftBlock;
		}
