...
public interface IWrenchable {
    ...
    public default boolean onWrenched(World world, int x, int y, int z, int face, EntityPlayer player) {
		...
		if (world.getBlockMetadata(x, y, z) != rotatedMeta)
    ...
    public default int getRotatedBlockMeta(World world, int x, int y, int z, int face) {
		...
		if (Direction.from3DDataValue(face).getAxis() == Axis.Y && block instanceof HorizontalAxisKineticBlock) {
			return this.getMetaFromDirection(this.getDirection(originalMeta).rotateAround(Axis.Y));
		}

		if (block instanceof RotatedPillarKineticBlock)
			return this.getMetaFromDirection(this.getDirection(originalMeta).rotateAround(Direction.from3DDataValue(face).getAxis()));

		if (!(block instanceof DirectionalKineticBlock))
			return originalMeta;

		Direction originalFacing = this.getDirection(originalMeta);
		Axis rotationAxis = Direction.from3DDataValue(face).getAxis();
...
//			else
			...
			do {
				newMeta = this.getMetaFromDirection(this.getDirection(newMeta).rotateAround(rotationAxis));
...
//					newMeta = newMeta.cycle(DirectionalAxisKineticBlock.AXIS_ALONG_FIRST_COORDINATE);
			} while (this.getDirection(newMeta).getAxis() == rotationAxis);
		}
		return newMeta;
	}
}
