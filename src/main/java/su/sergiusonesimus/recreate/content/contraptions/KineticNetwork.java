...
import su.sergiusonesimus.recreate.content.contraptions.components.FlywheelTileEntity;
...
public class KineticNetwork {
    ...
    public void addSilently(KineticTileEntity te, float lastCapacity, float lastStress) {
        if (members.containsKey(te)) return;
        if (te.isSource()) {
    ...
    public void add(KineticTileEntity te) {
        if (members.containsKey(te)) return;
        if (te.isSource()) sources.put(te, te.calculateAddedStressCapacity());
    ...
    public void remove(KineticTileEntity te) {
        if (!members.containsKey(te)) return;
        if (te.isSource()) sources.remove(te);
        members.remove(te);
        te.updateFromNetwork(0, 0, 0);

        if (members.isEmpty()) {
            TorquePropagator.networks.get(te.getWorldObj())
                .remove(this.id);
            return;
        }
    ...
    public void sync() {
        for (KineticTileEntity te : members.keySet()) updateFromNetwork(te);
    }
    ...
    private void updateFromNetwork(KineticTileEntity te) {
        ...
        if (!wasOverStressed && te.isOverStressed() && te.getTheoreticalSpeed() != 0) {
            // TODO
            // AllTriggers.triggerForNearbyPlayers(AllTriggers.OVERSTRESSED, te.getWorldObj(), te.getBlockPos(), 4);
            // if (containsFlywheel)
            // AllTriggers.triggerForNearbyPlayers(AllTriggers.OVERSTRESS_FLYWHEEL, te.getWorldObj(), te.getBlockPos(),
            // 4);
        }
    }
