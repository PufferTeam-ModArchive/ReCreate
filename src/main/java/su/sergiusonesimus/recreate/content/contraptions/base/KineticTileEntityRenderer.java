...
public abstract class KineticTileEntityRenderer extends TileEntitySpecialRenderer {
	...
	protected static float getRotationOffsetForPosition(KineticTileEntity te, int x, int y, int z, final Axis axis) {
		...
		double d = (((axis == Axis.X) ? 0 : x) + ((axis == Axis.Y) ? 0 : y)
			+ ((axis == Axis.Z) ? 0 : z)) % 2;
		if (d == 0) 
			offset = 22.5f;
		return offset;
...
//		} else {
			...
			if (overStressedEffect != 0) {
				if (overStressedEffect > 0)
					result = Color.WHITE.mixWith(Color.RED, overStressedEffect);
				else
					result = Color.WHITE.mixWith(Color.SPRING_GREEN, -overStressedEffect);
			}
//		}
		return result;
	}
